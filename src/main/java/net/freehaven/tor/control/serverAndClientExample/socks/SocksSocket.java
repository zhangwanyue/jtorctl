...
public class SocksSocket extends Socket {
	...
	public SocksSocket(SocketAddress proxy, int connectToProxyTimeout,
	...
	private void receiveConnectResponse(InputStream in) throws IOException {
		...
		if (addressType == 1) IoUtils.read(in, ipv4); // IPv4
		else if (addressType == 4) IoUtils.read(in, ipv6); // IPv6
		else throw new IOException("Unsupported address type: " + addressType);
		IoUtils.read(in, port); // Port number
//		System.out.println("[ipv4]BND.ADDR: " + new String(ipv4) + " [ipv6]BND.ADDR: " + new String(ipv6) + " BND.PORT: " + new String(port));
	}
}
